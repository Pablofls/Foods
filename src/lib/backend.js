...
const SEED = () => ({
  meals: [
    ...
    ['Sopa de fideo', 'Sopa', ['Fideo', 'Jitomate', 'Caldo de pollo', 'Cebolla'], false, 10, 6],
    ['Caldo de pollo con verduras', 'Sopa', ['Pollo', 'Zanahoria', 'Calabaza', 'Chayote', 'Arroz'], false, 5, 14],
    ['Pescado empapelado', 'Pescado', ['Filete de pescado', 'Jitomate', 'Cebolla', 'Limón', 'Mantequilla'], false, 3, 20],
    ['Chiles rellenos', 'Vegetariano', ['Chile poblano', 'Queso', 'Huevo', 'Jitomate', 'Harina'], false, 4, 18],
    ['Tacos dorados de papa', 'Antojito', ['Tortillas', 'Papa', 'Lechuga', 'Crema', 'Queso'], true, 6, 9],
    ['Fajitas de pollo', 'Pollo', ['Pechuga de pollo', 'Pimiento', 'Cebolla', 'Tortillas'], false, 7, 4],
    ['Calabacitas con queso', 'Vegetariano', ['Calabaza', 'Elote', 'Jitomate', 'Queso', 'Cebolla'], false, 5, 11],
    ['Bistec encebollado', 'Res', ['Bistec de res', 'Cebolla', 'Papa', 'Salsa de soya'], false, 6, 7],
    ['Lentejas', 'Sopa', ['Lentejas', 'Tocino', 'Plátano macho', 'Jitomate'], false, 4, 16],
    ...
    ['Sopa de tortilla', 'Sopa', ['Tortilla', 'Jitomate', 'Chile pasilla', 'Aguacate', 'Queso'], false, 5, 15],
  ...
  plan_entries: {},   // { [day]: { meal_id, eaten } }
  manual_items: [],
  checked_ingredients: [],
  _seq: 1000,
});
...
const localBackend = {
  async loadAll() {
    ...
    return {
      meals: s.meals,
      planEntries: Object.entries(s.plan_entries).map(([day, v]) => ({ day, ...v })),
  ...
  async upsertPlan(day, entry) {
    lsMutate(s => { s.plan_entries[day] = { meal_id: entry.meal_id, eaten: entry.eaten }; });
  },
...
const supaBackend = {
  async loadAll() {
    const [meals, plans, manual, checked] = await Promise.all([
    ...
    for (const r of [meals, plans, manual, checked]) if (r.error) throw r.error;
    return {
      meals: meals.data,
  ...
  async upsertPlan(day, entry) {
    const { error } = await supabase.from('plan_entries').upsert({ day, meal_id: entry.meal_id, eaten: entry.eaten });
    if (error) throw error;
  },
  async deletePlan(day) {
