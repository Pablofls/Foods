// AddMealSheet.jsx — registrar nueva comida (portado de sheets.jsx).
import { useState, useEffect } from 'react';
import { CATS, catColor } from '../lib/constants';
import { Icon } from '../components/Icon';
import { Sheet, Btn, OrderNote } from '../components/primitives';

export function AddMealSheet({ open, onClose, onSave, presetName = '' }) {
  const [name, setName] = useState('');
  const [cat, setCat] = useState('Pollo');
  const [ings, setIngs] = useState(['']);

  useEffect(() => {
    if (open) { setName(presetName); setCat('Pollo'); setIngs(['']); }
  }, [open, presetName]);
  ...
  const save = () => {
    ...
    if (!name.trim()) return;
    onSave({ name: name.trim(), category: cat, ingredients: cat === 'Restaurante' ? [] : clean });
    onClose();
  };
  ...
  const labelStyle = { fontFamily: 'var(--sans)', fontSize: 12.5, fontWeight: 700, letterSpacing: 0.5, textTransform: 'uppercase', color: 'var(--ink-40)', display: 'block', marginBottom: 8 };

  return (
    <Sheet open={open} onClose={onClose} title="Nueva comida">
      <div style={{ marginBottom: 18 }}>
        <label style={labelStyle}>Nombre del platillo</label>
        <input value={name} onChange={e => setName(e.target.value)} placeholder="Ej. Sopa de verduras" style={inputStyle} autoFocus />
      </div>
          ...
          {cats.map(c => {
            ...
            return (
              <button key={c} onClick={() => setCat(c)} style={{
                ...
                borderRadius: 999, padding: '8px 13px', fontFamily: 'var(--sans)', fontSize: 13.5, fontWeight: 600, cursor: 'pointer',
      ...
      {cat === 'Restaurante' ? (
        ...
        <div style={{ marginBottom: 22 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {ings.map((ing, i) => (
              <div key={i} style={{ display: 'flex', gap: 8 }}>
                <input value={ing} onChange={e => setIng(i, e.target.value)} placeholder={`Ingrediente ${i + 1}`} style={inputStyle}
                  onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); if (i === ings.length - 1) addIng(); } }} />
                {ings.length > 1 && (
                  <button onClick={() => rmIng(i)} style={{ border: 'none', background: 'var(--cream-2)', borderRadius: 13, width: 46, flexShrink: 0, cursor: 'pointer', color: 'var(--ink-40)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Icon name="close" size={17} sw={2.2} /></button>
                )}
              </div>
            ))}
          </div>
          <button onClick={addIng} style={{ marginTop: 10, border: '1.5px dashed var(--line)', background: 'transparent', borderRadius: 12, padding: '10px', width: '100%', cursor: 'pointer', color: 'var(--blue)', fontFamily: 'var(--sans)', fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
            <Icon name="plus" size={17} sw={2.2} stroke="var(--blue)" /> Otro ingrediente
          </button>
        </div>
      )}

      <Btn variant="primary" full onClick={save} icon="check">Guardar en mi recetario</Btn>
    </Sheet>
  );
}
